...
(оптимальная версия - [7z920.exe](http://www.7-zip.org/a/7z920.exe), 
но также автоопределяются и поддерживаются все версии, начиная с 4.65, 
x86 и x64, или же указать местоположение уже имеющегося в системе 
распаковщика в `PTK_PSD_Browser.ini`.
